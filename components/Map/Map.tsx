...
class Map extends React.Component<Props,
    ...
    addClick = points => {
        ...
        this.map.on('click', 'clusters', e => {
            ...
            this.map.getSource(this.pointsLayerName).getClusterExpansionZoom(
                clusterId,
                (err, zoom) => {
                    if (err) return;
                    if (!zoom) return;
        ...
        this.map.on("click", event => {
            ...
            if (this.map.queryRenderedFeatures(event.point)
                .filter(feature => feature.source === this.pointsLayerName).length === 0) {
